...
int main()
{
    ...
    do
    {
        ...
        printf("----------------------------------------------------------------\n");
        printf("Command = ");
        scanf(" %c", &command); //command 값 scanf
        switch (command)
        {
        case 'i':
        case 'I':
            getInfix(); //조건에 맞는 함수 출력 (switch문 아래로 쭉)
            break;
        case 'p':
        case 'P':
            toPostfix();
...
precedence getToken(char symbol)
{
    switch (symbol) //switch문의 조건에 맞는 값을 return한다.
    {
    case '(':
        return lparen;
    case ')':
...
void toPostfix()
{
    ...
    while (*exp != '\0')
    {
        if (getPriority(*exp) == operand)  //while문과 while문 안에 위치한 if문의 조건에 맞는 값을 출력한다.
        {
            x = *exp;
            charCat(&x);
        }
...
{
    printf("\n---DEBUG\n");
    printf("infixExp = %s\n", infixExp); //infixExp 값 출력 
    printf("postExp = %s\n", postfixExp); //postfixExp 값 출력
    printf("eval result = %d\n", evalResult); //evalResult 값 출력
    printf("postfixStack : ");
    for (int i = 0; i < MAX_STACK_SIZE; i++)
        printf("%c ", postfixStack[i]); //for 조건문에 맞게 postficStack 출력
    printf("\n");
}
void reset()
{
...
void evaluation()
{
    ...
    char symbol;
    evalStackTop = -1;
    for (i = 0; i < length; i++) //for문과 if문의 조건에 맞는 값 출력
    {
        symbol = postfixExp[i];
